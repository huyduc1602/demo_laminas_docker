...
use stdClass;
class MinifyHeadLink extends HeadLink {
	/**
	 * 
	 ...
	 */
	public function __invoke(array $attributes = null, $placement = \Laminas\View\Helper\Placeholder\Container::APPEND) {
	...
	public function toString($indent = null) {
		...
		foreach ( $this as $item ) {
			...
			if ( ($item->type ?? '')== 'text/css' 
			    ...
			    && $this->isValidStyleSheetExtension($item->href)
		    ) {
				$stylesheets [$item->media] [] = $item->href;//str_replace($this->getBaseUrl(), '', $item->href);
			} else {
		...
		return $indent . implode(
		    $this->escape($this->getSeparator()) . $indent, $items
	    );
