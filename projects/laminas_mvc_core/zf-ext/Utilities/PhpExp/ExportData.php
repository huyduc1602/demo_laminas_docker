...
/**
 * ExportData is the base class for exporters to specific file formats.
 * See other
 * classes below.
 */
...
abstract class ExportData
{
    ...
    protected $exportTo;
 // Set in constructor to one of 'browser', 'file', 'string'
    protected $stringData;
 // stringData so far, used if export string mode
    protected $tempFile;
 // handle to temp file (for export file mode)
    protected $tempFilename;
 // temp file name and path (for export file mode)
    public $filename;
 // file mode: the output file name; browser mode: file name for download; string mode: not used
    public $csv_separator;

    public function __construct($exportTo = "browser", $filename = "exportdata", $csv_separator = ',')
    {
        if (! in_array($exportTo, array(
            'browser',
            'file',
            'string'
        ))) {
